...
void cargararchivoincidente()
{
    ...
    strcpy(i.nropoliza,"014");
    strcpy(i.dniasegurado,"987654");
    strcpy(i.dniotroconductor,"545664");
...
void sobreescribirArchivo(nodopoliza *&a)
{
    FILE *f;
    poliza r;
...
void guardarPolizasArchivo(nodopoliza *lp)
{
    ...
    if (arch_polizas= fopen("Asegurados.BAK","wb"))
    {
        ...
        while (auxPol)
        {
            ...
            auxPol= obtenersiguientepoliza(auxPol);
        }
        fclose(arch_polizas);
    }
    else
...
void guardarIncidentesArchivo(nodoincidente *li)
{
    FILE *arch_incidente;
    nodoincidente *auxInc=NULL;
    if (arch_incidente= fopen("procesados.BAK","wb"))
    {
        auxInc= li;
        while (auxInc)
        {
            fwrite(&(auxInc->info), sizeof(incidente),1,arch_incidente);
            auxInc= obtenersiguienteincidente(auxInc);
        }
        fclose(arch_incidente);
    }
    else
    ...
    do
    {
        ...
        switch (elegida)
        {
            ...
            case '6':
                ...
                r=buscarpordni(listapolizas,i.dniasegurado);
                if(r)
                {
            ...
            case '9':
                ...
                sobreescribirArchivo(listapolizas);
                guardarPolizasArchivo(listapolizas);
                guardarIncidentesArchivo(listaincidentes);
                borrarlistapoliza(listapolizas);
                levantararchivos(listapolizas);
            break;
        }
